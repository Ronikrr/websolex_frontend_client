...
function Banner() {
    ...
    useEffect(() => {
        if (projectError || staticError) {
            setFeedback({ message: `Error: ${projectError?.message || staticError?.message}`, type: "error" });
        }
    }, [projectError, staticError]);
    ...
    return (
        <>
            {feedback && <FeedbackMessage message={feedback.message} type={feedback.type} onClear={() => setFeedback(null)} />}
            <section className='banner2'>
                <div className="container">
                    <div className="row">
                        <div className="col-12 d-flex flex-wrap">
                            <div className="col-12 col-lg-7 col-xl-6 mb-5 mb-lg-0">
                                <div className="banner_content2 mt-5 text-center text-lg-start wow animate__fadeInLeft">
                                    <h2 className='text-light fs-1 mb-5 mb-lg-2'>Empowering Your Digital Transformation</h2>
                                    <span className='text-light d-inline d-lg-none fs-lg-6 '>At WebSolex Infotech, we specialize in delivering cutting-edge technology solutions tailored to your needs.</span>
                                    <span className='text-light text_p d-none d-lg-block  fs-lg-6 '>At WebSolex Infotech, we specialize in delivering cutting-edge technology solutions that drive innovation and efficiency. From website development to application design, UI/UX services, graphics design, and digital marketing, we provide a full suite of IT services tailored to your needs. Let's build your digital future together.</span>
                                    <div className="banner_button2 d-flex mt-4 pb-5 border-bottom justify-content-center justify-content-lg-start">
                                        <Link to="/contact" className='text-decoration-none text-light bg-light text-dark rounded-pill me-3 border hover_button1'>Get Started</Link>
                                    </div>
                                    {/* <div className="col-12 d-flex flex-wrap justify-content-center text-center mt-3">
                                        <div className="col-6 col-sm-4">
                                            <h2>{statics?.successfulproject || 0}%</h2>
                                            <p>Successful Project</p>
                                        </div>
                            ...
                            <div className="col-12 d-none d-md-block col-lg-5 col-xl-6">
                                <div className="image_sub_box d-flex justify-content-center position-relative wow animate__fadeInRight">
                                    <img loading='lazy' src={banner_img} alt="Banner" className='col-12 col-md-9 col-xl-8 rounded-4' />
                                    <div className="review_main_box position-absolute bg-light p-3 d-flex align-items-center justify-content-between rounded-pill col-xxl-6">
                                        <div className="note-images d-flex justify-content-center me-3">
                                            {[review_img1, review_img2, review_img3, review_img4].map((img, index) => (
                                                <div key={index} className="note-img rounded-circle bg-light overflow-hidden">
                                                    <img loading='lazy' src={img} alt="Review" className='border-0' />
                                                </div>
                                            ))}
                                        </div>
                                        <div className="review_contentt">
                                            <h6 className='mb-0'>10+ Reviews</h6>
                                            <ul className='d-flex ps-0 mb-0'>
                                                {[...Array(5)].map((_, index) => (
                                                    <li key={index} className='pe-2'><FaStar className='icon_color' /></li>
                                                ))}
                                            </ul>
                                        </div>
                                    </div>
                                    <div className="view_sec d-flex justify-content-between position-absolute bg-light p-3 rounded-4 col-xxl-6">
                                        <div className="text-center">
                                            <span className='fw-semibold'>Total Project</span>
                                            <h2 className='mb-0'>{project?.completedProjects}+</h2>
                                        </div>
                                        <img loading='lazy' src={graf2} alt="Graph" className='col-9' />
                                    </div>
                                    <div className="sale_report d-flex position-absolute bg-light p-3 rounded-4 col-xxl-6">
                                        <div>
                                            <span className='px-2 py-1 text-light rounded-pill bg-primary'>New</span>
                                            <h6 className='mb-0 mt-2'>Sale Report</h6>
                                            <p className='mb-0'>Real-Time Sales Analytics</p>
                                        </div>
                                        <img loading='lazy' src={report_graf} alt="Report" className='col-9' />
                                    </div>
                                </div>
                            </div>
                        </div>
